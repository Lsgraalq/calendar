...
export default function CalendarPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribeEvents = onSnapshot(eventsQuery, (snapshot) => {
      ...
      snapshot.forEach(doc => {
        ...
        eventsArr.push({
          id: doc.id,
          title: data.type === 'booking' ? `Reservierung: ${data.equipmentName}` : data.title,
          start: data.start,
          end: data.end,
          type: data.type,
          ...
          color: data.type === 'booking' ? 'blue' : data.type === 'note' ? 'orange' : 'yellow',
        });
      });
      setEvents(eventsArr);
      setLoading(false);
  ...
  const handleAddEvent = async (e: FormEvent) => {
    ...
    if (formData.type !== 'booking' && !formData.title) {
    ...
    try {
      await addDoc(collection(db, 'events'), {
        title: formData.type === 'booking' ? '' : formData.title,
      ...
      <div className='pt-20 flex flex-col-reverse md:flex-row'>
        ...
        <div className='text'>
          ...
          <form onSubmit={handleAddEvent}>
            ...
            {/* Titel скрываем для брони техники */}
